...
export const membershipApi = {
  listTiers: () =>
    apiClient.get<{ success: boolean; data: MembershipTier[] }>(
  ...
  createTier: (data: {
    ...
    apiClient.post<{ success: boolean; data: MembershipTier }>(
      "/api/membership/tiers",
      data,
    ),
  updateTier: (
    ...
    apiClient.put<{ success: boolean; data: MembershipTier }>(
      `/api/membership/tiers/${id}`,
      data,
    ),
};
...
export const couponApi = {
  ...
  update: (
    ...
    apiClient.put<{ success: boolean; data: Coupon }>(
      `/api/coupons/${id}`,
      data,
    ),
  validate: (data: {
...
export const storedValueApi = {
  getBalance: (studentId: string) =>
    apiClient.get<{ success: boolean; data: StoredValueBalance }>(
      `/api/stored-value/${studentId}/balance`,
    ),
  topUp: (studentId: string, data: { amount: number; note?: string }) =>
    apiClient.post<{
      success: boolean;
      data: { balance: number; transaction: StoredValueTransaction };
    }>(`/api/stored-value/${studentId}/top-up`, data),
  spend: (
    studentId: string,
    data: { amount: number; referenceId: string; referenceType: string },
  ) =>
    apiClient.post<{
      success: boolean;
      data: { balance: number; transaction: StoredValueTransaction };
    }>(`/api/stored-value/${studentId}/spend`, data),
  listTransactions: (
    studentId: string,
    ...
    apiClient.get<{
      ...
      data: {
        ...
        page: number;
        limit: number;
        items: StoredValueTransaction[];
      };
    }>(`/api/stored-value/${studentId}/transactions`, { params }),
  getReceipt: (transactionId: string) =>
    apiClient.get<{ success: boolean; data: string }>(
      `/api/stored-value/transactions/${transactionId}/receipt`,
    ),
};
