...
import {
  listStudents, getStudentById, createStudent, updateStudent,
  deactivateStudent, exportStudentsCsv,
...
export async function createStudentHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
...
export async function updateStudentHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const dto     = updateStudentSchema.parse(req.body);
    const student = await updateStudent(req.params.id, dto, req.user!.id, req.user!.role);
    res.json({ success: true, data: student });
  } catch (err) { next(err); }
}

export async function deactivateStudentHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
...
export async function importStudentsHandler(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.file) {
      res.status(400).json({ success: false, error: 'No file uploaded', code: 'MISSING_FILE' });
      return;
    }
    ...
    if (!allowedExts.includes(ext)) {
      res.status(400).json({ success: false, error: 'Only .xlsx and .csv files are accepted', code: 'INVALID_FILE_TYPE' });
      return;
    }
    ...
    await import('../jobs/job.service').then(m => m.updateJobRecord(jobRecord.id, { bullJobId: String(bullJob.id) }));

    res.status(202).json({
      success: true,
      data: { jobId: jobRecord.id, status: 'waiting', message: 'Import queued — poll GET /api/jobs/:jobId for status' },
    });
  } catch (err) { next(err); }
}
