...
export const updateTicketStatusSchema = z.object({
  status: z.enum(['open', 'under_review', 'pending_approval', 'resolved', 'closed']),
...
export async function createTicket(
  ...
  await writeAuditEntry(
    ...
    'AfterSalesTicket',
    ticket.id,
    { type: data.type, studentId: data.studentId, slaDeadlineAt: slaDeadlineAt.toISOString() },
  );

  logger.info({ msg: 'After-sales ticket created', ticketId: ticket.id, type: data.type, actorId });
  ...
  if (data.type === 'delay') {
    try {
      ...
      logger.warn({ msg: 'Auto compensation suggestion failed', ticketId: ticket.id, err: compErr });
    }
  }
...
export async function updateTicketStatus(
  ...
  if (!allowed.includes(status)) {
    const err: any = new Error(`Cannot transition from '${ticket.status}' to '${status}'`);
    err.status = 422;
  ...
  logger.info({ msg: 'Ticket status updated', ticketId: id, from: ticket.status, to: status });
  return updated;
}
