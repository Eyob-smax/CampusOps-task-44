...
export async function listShipments(params: {
  ...
  const where: any = {};
  if (params.fulfillmentRequestId) where.fulfillmentRequestId = params.fulfillmentRequestId;
...
export async function createShipment(
  ...
  if (existing) {
    const err: any = new Error('A shipment already exists for this fulfillment request');
    err.status = 409;
  ...
  await writeAuditEntry(
    ...
    'Shipment',
    shipment.id,
    { fulfillmentRequestId: data.fulfillmentRequestId, carrierId: data.carrierId, warehouseId: data.warehouseId },
  );
...
export async function updateShipmentStatus(
  ...
  if (!allowed.includes(status)) {
    const err: any = new Error(`Cannot transition from '${shipment.status}' to '${status}'`);
    err.status = 422;
  ...
  const updated = await prisma.shipment.update({
    where: { id },
    data:  updateData,
    include: { carrier: true, warehouse: true, fulfillmentRequest: true, parcels: true },
  });
  ...
  await writeAuditEntry(
    ...
    'Shipment',
    id,
    { from: shipment.status, to: status },
  );
  ...
  logger.info({ msg: 'Shipment status updated', shipmentId: id, from: shipment.status, to: status });
  return updated;
}
