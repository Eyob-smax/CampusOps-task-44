...
export async function createZone(data: z.infer<typeof createZoneSchema>) {
  return prisma.deliveryZone.create({ data, include: { zipCodes: true, shippingTemplates: true } });
}

export async function updateZone(id: string, data: z.infer<typeof updateZoneSchema>) {
...
export async function addZipCode(zoneId: string, data: z.infer<typeof addZipSchema>) {
  ...
  return prisma.deliveryZoneZip.upsert({
    ...
    create: { zoneId, zipCode: data.zipCode, isNonServiceable: data.isNonServiceable ?? false },
  });
}
...
export async function removeZipCode(zoneId: string, zipCode: string) {
  ...
  await prisma.deliveryZoneZip.delete({ where: { zoneId_zipCode: { zoneId, zipCode } } });
  return { deleted: true };
}

export async function setZipNonServiceable(zoneId: string, zipCode: string, isNonServiceable: boolean) {
  await getZoneById(zoneId);
