...
interface RowError { row: number; studentNumber: string; errors: string[] }
...
function writeErrorReport(jobRecordId: string, errors: RowError[]): string {
  ...
  const lines = errors.map(e => [e.row, csvEscape(e.studentNumber), csvEscape(e.errors.join('; '))].join(','));
...
export const importWorker = new Worker(
  ...
  async (bullJob) => {
    const { jobRecordId, filePath, actorId } = bullJob.data as {
      jobRecordId: string; filePath: string; actorId: string;
    };
    ...
    await prisma.jobRecord.update({
      ...
      data:  { status: 'active', startedAt: new Date(), bullJobId: String(bullJob.id) },
    });
    ...
    await prisma.jobRecord.update({ where: { id: jobRecordId }, data: { totalRows: rawRows.length } });
    ...
    const [departments, membershipTiers, allStudents] = await Promise.all([
      ...
      prisma.student.findMany({ select: { studentNumber: true, email: true, id: true } }),
    ]);
    ...
    type ValidRow = z.infer<typeof importRowSchema> & { departmentId?: string; membershipTierId?: string };
    ...
    for (let i = 0; i < rawRows.length; i++) {
      ...
      const parsed = importRowSchema.safeParse({
        studentNumber:  resolveField(raw, 'studentNumber', 'student_number', 'Student Number', 'StudentNumber'),
        ...
        departmentCode: resolveField(raw, 'departmentCode', 'department_code', 'Department Code', 'DeptCode'),
        membershipTier: resolveField(raw, 'membershipTier', 'membership_tier', 'Membership Tier', 'MembershipTier'),
      });
      ...
      if (existingStudentNum && existingStudentNum !== data.studentNumber) {
        rowErrors.push(`Email ${data.email} is already registered to student ${existingStudentNum}`);
      }
    ...
    notifyJobProgress(jobRecordId, 'active', 20, { validRows: validRows.length, invalidRows: errors.length });
    ...
    for (let b = 0; b < validRows.length; b += BATCH) {
      ...
      await Promise.all(batch.map(async (row, idx) => {
        try {
          ...
          failed++;
          errors.push({ row: b + idx + 2, studentNumber: row.studentNumber, errors: [msg] });
        }
      }));

      const progress = 20 + Math.floor(((b + batch.length) / validRows.length) * 75);
      await bullJob.updateProgress(progress);
      await prisma.jobRecord.update({
        where: { id: jobRecordId },
        data: { progress, processedRows: created + updated, failedRows: failed + errors.length },
      });
      notifyJobProgress(jobRecordId, 'active', progress, { created, updated, failed });
    }
    ...
    // ---- 6. Finalize ----
    const result = JSON.stringify({ created, updated, failed, totalErrors: errors.length, errorReportPath });
    await prisma.jobRecord.update({
      where: { id: jobRecordId },
      data: {
        status:        'completed',
    ...
    notifyJobProgress(jobRecordId, 'completed', 100, { created, updated, failed: errors.length, errorReportPath });
    logger.info({ msg: 'Import worker completed', jobRecordId, created, updated, failed: errors.length });
  },
  connection,
);
...
importWorker.on('failed', async (bullJob, err) => {
  ...
  try {
    ...
    logger.error({ msg: 'Failed to update JobRecord on worker failure', jobRecordId, err: updateErr });
  }
});
