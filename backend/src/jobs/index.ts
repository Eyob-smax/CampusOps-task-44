...
export const shipmentSyncQueue   = new Queue('campusops:shipment-sync',      connection);
export const escalationQueue     = new Queue('campusops:escalation-checker', connection);
export const heartbeatQueue      = new Queue('campusops:heartbeat-checker',  connection);
export const parkingSlaQueue     = new Queue('campusops:parking-sla-check',  connection);
export const backupQueue         = new Queue('campusops:backup',             connection);
export const alertCheckQueue     = new Queue('campusops:metric-alert-check', connection);
 ...
 */
export function notifyJobProgress(jobId: string, status: string, progress?: number, data?: unknown): void {
  emitToNamespace('/jobs', 'job:update', { jobId, status, progress, data, timestamp: new Date().toISOString() });
}
...
function registerWorkerStubs(): void {
  ...
  for (const { queue, name } of queues) {
    new Worker(queue, async (job) => {
      logger.debug({ msg: `Job worker stub executed`, job: name, jobId: job.id });
    }, connection);
  }
}
