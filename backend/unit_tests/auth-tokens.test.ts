...
describe('access token payload', () => {
  function issueAccessToken(user: { id: string; username: string; role: string }, jti: string): string {
    return jwt.sign(
  ...
  it('contains id claim', () => {
    const token = issueAccessToken({ id: 'u-1', username: 'alice', role: 'administrator' }, 'jti-1');
  ...
  it('contains role claim', () => {
    const token = issueAccessToken({ id: 'u-1', username: 'alice', role: 'operations_manager' }, 'jti-1');
    const decoded = jwt.verify(token, TEST_SECRET) as any;
    expect(decoded.role).toBe('operations_manager');
  });

  it('contains jti claim', () => {
    const token = issueAccessToken({ id: 'u-1', username: 'alice', role: 'administrator' }, 'jti-abc');
    const decoded = jwt.verify(token, TEST_SECRET) as any;
  ...
  it('exp claim is approximately 15 minutes in the future', () => {
    ...
    expect(decoded.exp).toBeGreaterThanOrEqual(before + ACCESS_TOKEN_TTL_SECONDS - 1);
    expect(decoded.exp).toBeLessThanOrEqual(after + ACCESS_TOKEN_TTL_SECONDS + 1);
  });
...
describe('refresh token payload', () => {
  ...
  it('exp is approximately 8 hours in the future', () => {
    ...
    const decoded = jwt.verify(token, TEST_SECRET) as any;
    const after = Math.floor(Date.now() / 1000);
    expect(decoded.exp).toBeGreaterThanOrEqual(before + REFRESH_TOKEN_TTL_SECONDS - 1);
    expect(decoded.exp).toBeLessThanOrEqual(after + REFRESH_TOKEN_TTL_SECONDS + 1);
  });
...
describe('token pair', () => {
  function issueTokenPair(user: { id: string; username: string; role: string }) {
  ...
  it('access and refresh tokens are different strings', () => {
    const pair = issueTokenPair({ id: 'u-1', username: 'bob', role: 'auditor' });
  ...
  it('two token pairs for same user produce different access tokens', () => {
    const p1 = issueTokenPair({ id: 'u-1', username: 'carol', role: 'administrator' });
    const p2 = issueTokenPair({ id: 'u-1', username: 'carol', role: 'administrator' });
...
describe('JWT verification edge cases', () => {
  ...
  it('jwt.decode returns payload without verification', () => {
    const token = jwt.sign({ id: 'u-1', jti: 'test-jti', exp: 9999999999 }, TEST_SECRET);
...
describe('createUserSchema password rules', () => {
  const { createUserSchema } = await import('../src/modules/admin/user.service');
  ...
  it('rejects password shorter than 12 chars', () => {
    expect(createUserSchema.safeParse({ ...validPayload, password: 'Short1!' }).success).toBe(false);
  });

  it('rejects password without uppercase', () => {
    expect(createUserSchema.safeParse({ ...validPayload, password: 'alllowercase1!' }).success).toBe(false);
  });

  it('rejects password without lowercase', () => {
    expect(createUserSchema.safeParse({ ...validPayload, password: 'ALLUPPERCASE1!' }).success).toBe(false);
  });

  it('rejects password without digit', () => {
    expect(createUserSchema.safeParse({ ...validPayload, password: 'NoDigitHere!!' }).success).toBe(false);
  });

  it('rejects password without special char', () => {
    expect(createUserSchema.safeParse({ ...validPayload, password: 'NoSpecialChar1A' }).success).toBe(false);
  });

  it('rejects invalid role', () => {
    expect(createUserSchema.safeParse({ ...validPayload, role: 'superadmin' as any }).success).toBe(false);
  });

  it('accepts all valid roles', () => {
    const validRoles = ['administrator', 'operations_manager', 'classroom_supervisor', 'customer_service_agent', 'auditor'];
  ...
  it('rejects username shorter than 3 chars', () => {
    expect(createUserSchema.safeParse({ ...validPayload, username: 'ab' }).success).toBe(false);
  });

  it('rejects username with invalid characters', () => {
    expect(createUserSchema.safeParse({ ...validPayload, username: 'user name!' }).success).toBe(false);
  });

  it('accepts username with allowed special chars (_, ., -)', () => {
    expect(createUserSchema.safeParse({ ...validPayload, username: 'user_name.ok-here' }).success).toBe(true);
  });
});
