...
describe('simulateCarrierResponse — age-based status rules', () => {
  ...
  it('handles multiple parcels at different ages', () => {
    const now = new Date();
    const parcels = [
      { id: 'p1', createdAt: new Date(now.getTime() - 0.5 * 3600_000), status: 'pending' },
      { id: 'p2', createdAt: new Date(now.getTime() - 2   * 3600_000), status: 'pending' },
      { id: 'p3', createdAt: new Date(now.getTime() - 25  * 3600_000), status: 'pending' },
    ];
    const updates = simulateCarrierResponse(parcels, now);
...
describe('createTicketSchema', () => {
  ...
  it('rejects non-UUID studentId', () => {
    const result = createTicketSchema.safeParse({ ...base, studentId: 'not-a-uuid' });
  ...
  it('rejects non-UUID shipmentId', () => {
    const result = createTicketSchema.safeParse({ ...base, shipmentId: 'bad-id' });
    expect(result.success).toBe(false);
  });
});
...
describe('updateTicketStatusSchema', () => {
  it('accepts all valid statuses', () => {
    const statuses = ['open', 'under_review', 'pending_approval', 'resolved', 'closed'];
  ...
  it('accepts optional note', () => {
    const result = updateTicketStatusSchema.safeParse({ status: 'closed', note: 'Issue resolved' });
    expect(result.success).toBe(true);
  });
...
describe('Ticket SLA deadline calculation', () => {
  it('delay SLA: 72 hours', () => {
    const SLA_HOURS: Record<string, number> = { delay: 72, dispute: 48, lost_item: 96 };
  ...
  it('lost_item SLA: 96 hours', () => {
    const SLA_HOURS: Record<string, number> = { delay: 72, dispute: 48, lost_item: 96 };
    const now = new Date('2024-01-01T12:00:00Z');
    const deadline = new Date(now.getTime() + SLA_HOURS['lost_item']! * 3600_000);
...
describe('ticket status state machine', () => {
  ...
  it('open → under_review: valid', () => expect(canTransition('open', 'under_review')).toBe(true));
  it('open → closed: valid', () => expect(canTransition('open', 'closed')).toBe(true));
  it('open → pending_approval: invalid', () => expect(canTransition('open', 'pending_approval')).toBe(false));
  it('open → resolved: invalid', () => expect(canTransition('open', 'resolved')).toBe(false));
  it('under_review → pending_approval: valid', () => expect(canTransition('under_review', 'pending_approval')).toBe(true));
  it('under_review → open: invalid', () => expect(canTransition('under_review', 'open')).toBe(false));
  it('pending_approval → resolved: valid', () => expect(canTransition('pending_approval', 'resolved')).toBe(true));
  it('pending_approval → closed: valid', () => expect(canTransition('pending_approval', 'closed')).toBe(true));
  it('resolved → closed: valid', () => expect(canTransition('resolved', 'closed')).toBe(true));
  it('resolved → open: invalid', () => expect(canTransition('resolved', 'open')).toBe(false));
...
describe('compensation approval tiers', () => {
  ...
  it('limited tier: can approve $25', () => expect(canApprove('limited', 25)).toBe(true));
  it('limited tier: cannot approve $25.01', () => expect(canApprove('limited', 25.01)).toBe(false));
  it('limited tier: cannot approve $50', () => expect(canApprove('limited', 50)).toBe(false));
  it('full tier: can approve $50', () => expect(canApprove('full', 50)).toBe(true));
  it('full tier: can approve $25', () => expect(canApprove('full', 25)).toBe(true));
  it('full tier: cannot approve $50.01', () => expect(canApprove('full', 50.01)).toBe(false));
  ...
  it('unknown tier: cannot approve any amount', () => expect(canApprove('unknown', 1)).toBe(false));
});
...
describe('compensation cap: $50 global cap', () => {
  ...
  function computeFinalAmount(suggested: number, existingApproved: number, capAmount: number): number {
