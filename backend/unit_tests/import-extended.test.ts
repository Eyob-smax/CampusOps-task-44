...
const { createStudentSchema, updateStudentSchema } = await import('../src/modules/master-data/student.service');

// ---- createUserSchema from user.service.ts ----
const { createUserSchema, updateUserSchema } = await import('../src/modules/admin/user.service');
...
describe('resolveField utility', () => {
  ...
  it('returns value of first matching key', () => {
    expect(resolveField({ studentNumber: 'S001' }, 'studentNumber', 'student_number')).toBe('S001');
  });

  it('falls back to second key when first is absent', () => {
    expect(resolveField({ student_number: 'S002' }, 'studentNumber', 'student_number')).toBe('S002');
  });

  it('returns empty string when no key matches', () => {
    expect(resolveField({ other: 'X' }, 'studentNumber', 'student_number')).toBe('');
  });

  it('skips null values and tries next key', () => {
    expect(resolveField({ studentNumber: null, student_number: 'S003' }, 'studentNumber', 'student_number')).toBe('S003');
  });

  it('converts numeric values to string', () => {
    expect(resolveField({ studentNumber: 12345 }, 'studentNumber')).toBe('12345');
  });
});
