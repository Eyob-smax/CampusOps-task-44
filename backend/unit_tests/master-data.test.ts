...
describe("createDepartmentSchema", () => {
  it("accepts valid department", () => {
    expect(() =>
      createDepartmentSchema.parse({ name: "Computer Science", code: "cs" }),
    ).not.toThrow();
  });
  ...
  it("rejects empty name", () => {
    expect(() =>
      createDepartmentSchema.parse({ name: "", code: "CS" }),
    ).toThrow();
  });

  it("rejects code longer than 20 chars", () => {
    expect(() =>
      createDepartmentSchema.parse({ name: "X", code: "A".repeat(21) }),
    ).toThrow();
  });
});
...
describe("updateDepartmentSchema", () => {
  it("allows partial update", () => {
    expect(() =>
      updateDepartmentSchema.parse({ name: "New Name" }),
    ).not.toThrow();
  });
...
describe("createCourseSchema", () => {
  ...
  it("upcases code", () => {
    const c = createCourseSchema.parse({
      code: "cs101",
      name: "Intro",
      departmentId: deptId,
    });
...
describe("createStudentSchema", () => {
  ...
  it("normalises email to lowercase", () => {
    const s = createStudentSchema.parse({
      studentNumber: "S001",
      fullName: "Alice",
      email: "ALICE@EXAMPLE.COM",
    });
...
describe("updateStudentSchema", () => {
  it("allows partial update", () => {
    expect(() =>
      updateStudentSchema.parse({ phone: "+1-555-0100" }),
    ).not.toThrow();
  });
